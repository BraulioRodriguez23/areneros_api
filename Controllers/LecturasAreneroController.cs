...
public class LecturasAreneroController(AppDbContext db) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<List<LecturaArenero>>> Get([FromQuery] DateTime? desde, [FromQuery] DateTime? hasta)
    {
        var query = db.LecturasArenero.AsQueryable();
    ...
    [HttpPost]
    public async Task<ActionResult> Post([FromBody] List<LecturaArenero> lecturas)
    {
        if (lecturas.Count == 0) return BadRequest("No hay lecturas para guardar.");
        db.LecturasArenero.AddRange(lecturas);
